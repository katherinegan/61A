...
def about(topic):
    ...
    # END PROBLEM 2
    def inner(sentence):
        lower_sentence = remove_punctuation(sentence.lower())
        listed_sentence = lower_sentence.split()
        for i in topic:
            if i in listed_sentence:
                return True
        return False
    return inner
...
def accuracy(typed, reference):
    ...
    for i in reference_words:
        if len(typed_words) == 0:
            return 0.0
        if i == typed_words[count]:
            num += 1
        count += 1
        if count >= len(typed_words):
            break
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    element = min(valid_words, key=lambda k: diff_function(
        user_word, k, limit))
    if diff_function(user_word, element, limit) <= limit:
        return element
    else:
        return user_word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(len(player) - 1):
            differences.append(player[i + 1] - player[i])
...
def fastest_words(game):
    ...
    for i in players:
        ...
        for k in words:
            times = []
            for m in players:
                times.append(time(game, m, k))
            if min(times) == time(game, i, k) and not any(word_at(game, k) in best for best in results):
                best.append(word_at(game, k))
        results.append(best)
    return results
    # END PROBLEM 10
