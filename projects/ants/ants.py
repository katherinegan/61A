...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while location != beehive:
            if curr_range >= self.min_range and random_or_none(location.bees) != None and curr_range <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if self.imposter:
            ...
            while path != None:
                if path.ant != None:
                    if not path.ant.buffed:
                        path.ant.damage, path.ant.buffed = path.ant.damage * 2, True
                    if isinstance(path.ant, ContainerAnt) and path.ant.contained_ant != None and not path.ant.contained_ant.buffed:
                        path.ant.contained_ant.damage, path.ant.contained_ant.buffed = path.ant.contained_ant.damage * 2, True
                path = path.exit
        # END Problem 13
